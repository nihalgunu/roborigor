...
def write_tables(errs: list) -> None:
    ...
    r3_all = json.load(open(D / "failure_ratio_and_shift.json"))
    verdict_word = {"EQUIV": "equiv.", "EQUIVALENT": "equiv.", "DIFF": "worse", "DIFFERENT": "worse",
                    "INCONCLUSIVE": "inconcl."}

    def step_row(key, label, fr_key):
        r = st["rows"][key]
        lo, hi = r["ci_pts"]
        olo, ohi = r["cond_odds_ratio_ci"]
        fr = r3_fr[fr_key]
        if r["mcnemar_p"] < 0.05:
            v = "DIFF" if r["diff_pts"] < 0 else "better"
        else:
            v = "EQUIV" if r["equivalent_at_5"] else "INCONCLUSIVE"
        return (label, r["n_pairs"], r["a_only"], r["b_only"], r["diff_pts"], lo, hi, olo, ohi,
                fr["ratio"], fr["ci"][0], fr["ci"][1], verdict_word.get(v, v))

    def group_row(key, label):
        g = r3_all["camera_level_groups"][key]
        return (label, g["n"], g["s1_only"], g["s10_only"], g["diff_pts"], g["ci_pts"][0], g["ci_pts"][1],
                g["odds_ratio_ci"][0], g["odds_ratio_ci"][1], g["failure_ratio"],
                g["failure_ratio_ci"][0], g["failure_ratio_ci"][1], verdict_word[g["verdict"]])

    table = [step_row("pi05_battery", r"$\pi_{0.5}$ battery", "pi05_battery"),
             step_row("pi05_headroom_tasks", r"\quad informative tasks", "pi05_informative"),
             step_row("pi05_layout_shift", r"$\pi_{0.5}$ layout shift", "pi05_layout_shift"),
             group_row("levels_1_3", r"$\pi_{0.5}$ camera, levels 1--3"),
             group_row("levels_4_5", r"$\pi_{0.5}$ camera, levels 4--5"),
             step_row("smolvla_battery", r"SmolVLA battery", "smolvla_battery"),
             step_row("pi0_battery", r"$\pi_0$ battery", "pi0_battery")]
    trows = [f"{t[0]} & ${t[1]}$ & ${t[2]}$--${t[3]}$ & ${t[4]:+.1f}$ $[{t[5]:+.1f}, {t[6]:+.1f}]$ & "
             f"$[{t[7]:.2f}, {t[8]:.2f}]$ & ${t[9]:.2f}$ $[{t[10]:.2f}, {t[11]:.2f}]$ & {t[12]} \\\\"
             for t in table]
    ttbl = "\n".join([
        "% AUTO-GENERATED by scripts/check_paper_numbers.py -- do not edit.",
        r"\begin{tabular}{@{}lrrcccl@{}}",
        r"\toprule",
        r"Contrast & $n$ & $b$--$c$ & (ii) Difference, pts & (i) Odds ratio & (iii) Failure ratio & Verdict \\",
        r"\midrule",
        *trows,
        r"\bottomrule",
        r"\end{tabular}",
