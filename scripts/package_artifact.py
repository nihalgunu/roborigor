...
AUDIT_FILES = ["extraction.jsonl", "RESULTS.md", "recompute_final_50papers.json", "dial_reporting.jsonl"]
...
def copy_efficiency_audit(out: Path) -> list[Path]:
    """Protocol, candidate set, extraction, and verification of the efficiency-claim audit."""
    copied = []
    dst_dir = out / "efficiency_audit"
    dst_dir.mkdir(parents=True, exist_ok=True)
    sources = [REPO / "docs" / "efficiency-audit-protocol.md"] + [
        REPO / "docs" / "efficiency_audit" / name
        for name in ("candidates_union.jsonl", "extraction.jsonl", "verification_sample.json",
                     "sourceC_log.md", "EXTRACTOR_INSTRUCTIONS.md")]
    for src in sources:
        if not src.exists():
            raise FileNotFoundError(f"expected efficiency-audit file missing: {src}")
        dst = dst_dir / src.name
        shutil.copy2(src, dst)
        copied.append(dst)
    return copied
...
def main() -> int:
    ...
    files += copy_efficiency_audit(out)
    datasheet = out / "DATASHEET.md"
    shutil.copy2(REPO / "docs" / "DATASHEET.md", datasheet)
    files.append(datasheet)
