...
def fig_varstacks():
    ...
    for x, t in enumerate(tasks):
        sat = t["task_id"] in (1, 3, 4, 6)  # pre-declared saturated (configs/v1_varcomp_pi05.yaml)
        if sat:
            sat_x.append(x)
    ...
    ax.set_xticklabels([f"t{t['task_id']}\n{t['sr_observed']:.2f}".replace("0.", ".") for t in tasks],
                       fontsize=6)
...
def fig_transfer():
    """Fig. 1: (a) ceiling dilution of the step contrast, (b) the same contrast
    across policies, seeds, and shifts."""
    r = json.load(open(DATA / "step_transfer.json"))
    INK, BAND, WARN = "#1a1a1a", "#deebf7", "#8c510a"
    cen = json.load(open(DATA / "dilution_census.json"))
    fig, (ax1, ax3, ax2) = plt.subplots(1, 3, figsize=(7.1, 2.35), dpi=300,
                                        gridspec_kw={"width_ratios": [1, 0.8, 1.25]})

    # (a) admit tasks from most to least headroom.
    cur = r["dilution_curve_pi05"]
    xs = list(range(1, len(cur) + 1))
    ax1.axhspan(-5, 5, color=BAND, zorder=0)
    ax1.axhline(0, color="#999999", lw=0.6, zorder=1)
    for x, c in zip(xs, cur):
        lo, hi = c["ci_pts"]
        inside = lo > -5 and hi < 5
        col = INK if not inside else "#2171b5"
        ax1.plot([x, x], [lo, hi], color=col, lw=1.4, zorder=2)
        ax1.scatter([x], [c["diff_pts"]], s=14, color=col, zorder=3)
        ax1.text(x, hi + 1.0, f"{c['discordant']}", ha="center", fontsize=5.5, color=INK)
    ax1.set_xticks(xs)
    ax1.set_xticklabels([f"t{c['added_task']}\n{100 * c['added_task_ref_rate']:.0f}" for c in cur],
                        fontsize=5.5)
    ax1.set_xlabel("Task admitted (10-step success, %)")
    ax1.set_ylabel("1 step minus 10 steps (pts)")
    ax1.set_ylim(-19, 17)
    ax1.set_title("(a)", fontsize=6, loc="left", pad=2)
    ax1.text(0.98, 0.03, "labels: discordant pairs", transform=ax1.transAxes,
             fontsize=5.5, va="bottom", ha="right")

    # (c) census control: equivalence verdicts kept by each 4-of-8 task subset.
    ctl = cen["size_control"]
    total = cen["n_equiv_full"]
    kept = [row["kept"] for row in ctl["equiv_kept_by_subset"]]
    bins = range(0, total + 2)
    ax3.hist(kept, bins=bins, color="#c6dbef", edgecolor="#6baed6", lw=0.5, align="left")
    for val, lab, col in ((ctl["equiv_kept_saturated_half"], "saturated half", "#2171b5"),
                          (ctl["equiv_kept_informative_half"], "informative half", WARN)):
        ax3.axvline(val, color=col, lw=1.2, label=f"{lab} ({val})")
    ax3.legend(loc="lower center", bbox_to_anchor=(0.5, 1.0), ncol=1, fontsize=5.5, frameon=False)
    ax3.set_xlabel(f"Equivalences kept (of {total})")
    ax3.set_ylabel("4-task subsets")
    ax3.text(-0.02, 1.0, "(b)", transform=ax3.transAxes, fontsize=6, va="bottom", ha="right")

    # (b) forest.
    rows = [
        ("pi05_battery", "$\\pi_{0.5}$ battery (8 tasks)"),
        ("pi05_non_perfect_tasks", "  without the 100% task"),
        ("pi05_headroom_tasks", "  pre-classified informative"),
        ("pi05_envseed8", "$\\pi_{0.5}$ env seed 8"),
        ("pi05_layout_shift", "$\\pi_{0.5}$ layout shift"),
        ("pi05_camera_confirmatory", "$\\pi_{0.5}$ camera shift"),
        ("smolvla_battery", "SmolVLA battery"),
        ("pi0_battery", "$\\pi_0$ battery"),
    ]
    ax2.axvspan(-5, 5, color=BAND, zorder=0)
    ax2.axvline(0, color="#999999", lw=0.6, zorder=1)
    for i, (key, label) in enumerate(rows):
        row = r["rows"][key]
        y = len(rows) - 1 - i
        lo, hi = row["ci_pts"]
        inside = lo > -5 and hi < 5
        sig = row["mcnemar_p"] < 0.05
        col = "#2171b5" if inside else (WARN if sig else INK)
        ax2.plot([lo, hi], [y, y], color=col, lw=1.4, zorder=2)
        ax2.scatter([row["diff_pts"]], [y], s=14, color=col, zorder=3)
        ax2.text(10.2, y, f"n={row['n_pairs']}", va="center", fontsize=5.5, color=INK)
    ax2.set_yticks(range(len(rows)))
    ax2.set_yticklabels([lab for _, lab in rows][::-1], fontsize=6.5)
    ax2.set_xlim(-14, 13)
    ax2.set_xlabel("1 step minus 10 steps at horizon 10 (pts)")
    ax2.set_title("(c)", fontsize=6, loc="left", pad=2)
    fig.tight_layout(pad=0.4, w_pad=1.0)
    fig.savefig(OUT / "fig_transfer.pdf", bbox_inches="tight")
    print("fig_transfer.pdf")


def fig_efficiency():
    """Parity claims in VLA efficiency papers: base success vs claimed gap."""
    rows = [json.loads(line) for line in open(ROOT / "docs/efficiency_audit/extraction.jsonl") if line.strip()]
    pts = []
    for r in rows:
        c = r.get("claim")
        if not r["screen"]["include"] or not c or c.get("rate_base") is None or c.get("rate_method") is None:
            continue
        rep = bool(c.get("uncertainty_reported") or c.get("per_task_results") or c.get("paired_info"))
        pts.append((100 * c["rate_base"], 100 * (c["rate_method"] - c["rate_base"]), rep))
    fig, ax = plt.subplots(figsize=(3.4, 1.9), dpi=300)
    ax.axvspan(90, 100.5, color="#deebf7", zorder=0)
    ax.axhline(0, color="#999999", lw=0.6, zorder=1)
    for rep, col, lab in ((False, "#1a1a1a", "no uncertainty, per-task, or pairing"),
                          (True, "#2171b5", "reports one of these")):
        xs = [x for x, _, r_ in pts if r_ == rep]
        ys = [y for _, y, r_ in pts if r_ == rep]
        ax.scatter(xs, ys, s=9, color=col, alpha=0.8, lw=0, label=f"{lab} ({len(xs)})", zorder=2)
    ax.set_xlabel("Base policy success on LIBERO (%)")
    ax.set_ylabel("Method minus base (pts)")
    ax.set_ylim(-16, 8)
