...
def fig_hero():
    ...
    rows = [
        ...
        ("Replan every step\n(horizon 10$\\to$1)", horizon_cost, "dial", f"$-${horizon_cost:.1f}"),
...
def fig_pareto():
    ...
    fx, fy = [], []
    for c in sorted(t["cells"], key=lambda c: c["control_latency_ms_per_step"]):
        x, y = c["control_latency_ms_per_step"], c["success_rate"]
        lo, hi = c["ci95"]
    ...
    ax.minorticks_off()
    ax.set_xlabel("Control latency per step (ms, log)")
