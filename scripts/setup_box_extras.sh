...
#
#   ROBORIGOR_WHEEL=~/roborigor-*.whl bash setup_box_extras.sh      # wheel only
...
ROBORIGOR_WHEEL=${ROBORIGOR_WHEEL:-}

if [[ -n "$ROBORIGOR_WHEEL" ]]; then
  log "roborigor wheel into the LIBERO client venv"
  uv pip install --quiet --python examples/libero/.venv/bin/python "$ROBORIGOR_WHEEL" numpy
...
if [[ "${SETUP_LIBERO_PLUS:-0}" == "1" ]]; then
  ...
  if [[ -n "$ROBORIGOR_WHEEL" ]]; then
    uv pip install --quiet --python "$VP" "$ROBORIGOR_WHEEL"
  fi
...
if [[ "${SETUP_LEROBOT:-0}" == "1" ]]; then
  ...
  if [[ -n "$ROBORIGOR_WHEEL" ]]; then
    uv pip install --quiet --python "$LP" "$ROBORIGOR_WHEEL"
  fi
