...
def _cmd_report(args) -> int:
    ...
    def where(records, spec):
        """Keep records whose fields match every key=value in spec (values compared as text)."""
        if not spec:
            return records
        conds = [c.split("=", 1) for c in spec.split(",")]
        return [r for r in records if all(str(getattr(r, k)) == v for k, v in conds)]

    tasks = [int(t) for t in args.informative_tasks.split(",")] if args.informative_tasks else None
    rep = noloss_report(where(_records(args.a), args.a_where), where(_records(args.b), args.b_where),
                        informative_tasks=tasks,
                        margin_pts=args.margin, ratio_bound=args.ratio_bound)
    print("\n".join(rep.lines()))
    return 0
...
def main(argv=None) -> int:
    ...
    p = sub.add_parser("verify-seeding", help="check that per-request seed control works")
    p.add_argument("--host", default="127.0.0.1")
    p.add_argument("--port", type=int, default=8000)
    ...
    p = sub.add_parser("report", help="three-part report for a no-loss claim between two settings")
    p.add_argument("--a", nargs="+", required=True, help="records of setting A (e.g. the accelerated one)")
    p.add_argument("--b", nargs="+", required=True, help="records of setting B (the reference)")
    p.add_argument("--a-where", help="field filter for A, e.g. num_steps=1,exec_horizon=10")
    p.add_argument("--b-where", help="field filter for B, e.g. num_steps=10,exec_horizon=10")
    p.add_argument("--informative-tasks", help="comma-separated task ids declared informative in advance")
    p.add_argument("--margin", type=float, default=5.0, help="equivalence margin, points")
    p.add_argument("--ratio-bound", type=float, default=1.5, help="failure-rate ratio bound")
