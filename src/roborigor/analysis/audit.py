...
# Largest min(n) at which Boschloo's exact test is enumerated. Above this the
# scipy implementation's runtime becomes prohibitive; see deviations log
# Entry 16 for the timing measurements behind the value.
BOSCHLOO_MAX_N = 2000
...
def recompute(rows: list[dict], alpha: float = 0.05) -> dict:
    ...
    for row in rows:
        for c in row["comparisons"]:
            ...
            # Deviations log entries 1 and 16: Boschloo is the registered
            # primary test and is now enumerated wherever it terminates in
            # reasonable time (min(n) <= BOSCHLOO_MAX_N). Above that it is
            # genuinely intractable (superlinear in n; ~74 s at n = 2000 on
            # the reference machine), so Fisher is primary there and flagged.
            # Fisher is the conservative of the two: it returns the larger
            # p-value, so a Fisher substitution can only move a comparison
            # from significant to failing, never the reverse.
            if min(n, n_other) <= BOSCHLOO_MAX_N:
