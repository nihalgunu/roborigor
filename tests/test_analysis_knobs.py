...
def test_measured_latency_amortizes_over_executed_steps():
    """Measured latency is recorded server time per executed env step."""
    records = [
        rec(init_id=i, num_steps=10, exec_horizon=5, success=True,
            server_infer_s_total=1.0, n_env_steps=100, n_chunks=20)
        for i in range(10)
    ]
    cell = knob_table(records)["cells"][0]
    assert cell["measured_latency_ms_per_step"] == pytest.approx(10.0)
    assert cell["measured_chunk_ms"] == pytest.approx(50.0)


def test_measured_latency_is_none_without_timings():
    records = [
        rec(init_id=i, num_steps=1, exec_horizon=10, success=True,
            server_infer_s_total=0.0, n_env_steps=0, n_chunks=0)
        for i in range(5)
    ]
    cell = knob_table(records)["cells"][0]
    assert cell["measured_latency_ms_per_step"] is None
